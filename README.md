...
A Raspberry Pi emulator packaged in a [Docker image](https://hub.docker.com/r/ptrsr/pi-ci)
that allows developers to easily prepare and flash Raspberry Pi configurations,
streamlining setup and testing without physical hardware.
...
```zsh
docker build -t schubergphilis/pi-ci:v0.1.0-rc.1 .
```
...
docker run \
  --rm \
  -it \
  -v $PWD/dist:/dist schubergphilis/pi-ci:v0.1.0-rc.1 start
```
...
docker run \
  --rm \
  -it \
  -v ${PWD}/dist:/dist schubergphilis/pi-ci:v0.1.0-rc.1 export \
  --input /dist/distro.qcow2 \
